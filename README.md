...
> local `azai serve`. The hosted Worker is a protocol mirror + Lamb check,
...
Open http://127.0.0.1:8860 (loopback only). No CDN, no telemetry.
...
Direct tarball (also counted): [azai-0.1.0.tar.gz](https://azai-download-tracker.vibelock.workers.dev/download?asset=azai-0.1.0.tar.gz)
...
- Loopback UI, no telemetry. Hosted `/v1` never spends the author's
  paid keys and never proxies chat to GPT/Grok/Venice.
...
- Chat transcript + prompt box
- Model picker: `blend` | `gpt` | `grok` | `venice` | `local`
- Seal Runtime button
- Receipts rail (timestamp, action, result, hashes)
- Integrity: peace / clarity / service PASS / CHECK / FAIL
...
| POST | `/v1/chat/completions` | `messages[]`, `model`, `stream` accepted (v0.1 returns non-stream) |
...
Companion only: ask / read receipts / integrity / seal. Constitutional
edits are blocked. Dark gold. Offline against a local AZAI server.
...
```
azai/                 library (lamb, receipts, jeeves, providers, runtime, cli, ui)
