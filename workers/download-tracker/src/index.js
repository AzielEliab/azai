 ...
 * Suite mesh default OFF. QNS-CD-1.0 hub cite on status / Live Nodes.
 * Not a public qnsd proxy. Does not increment downloads or views.
 */
    ...
    <script>
      ...
      (function () {
        ...
        function paintMesh(raw) {
          ...
          else if (j.status === "unavailable" || (j.ok === false && j.error)) line.textContent = "Suite mesh: off (unavailable). QNM-BUILD-1.0. QNS-CD-1.0 cite. Not an anonymity network.";
          else line.textContent = "Suite mesh: off (default). QNM-BUILD-1.0. QNS-CD-1.0 cite. Not an anonymity network.";
          ...
          $("meshProducts").textContent = "Catalog MCP mesh_* · FragGate slug=mesh · QNS-CD-1.0 cite · /v1/mesh/* PROXY · not AnonBroadcast · not AZMail ring · not a Node Gate · not a Softwares-tab product · not a public qnsd proxy" + extra;
        }
...
<p>Suite mesh: <code>GET ${origin}/v1/mesh</code> PROXY to aziel-runtime. Default OFF. QNM-BUILD-1.0 live|locked|isolated. QNS-CD-1.0 hub cite (photon QNS1 packet transfer; local qnsd in qnm-node; runtime cites in aziel-runtime). No Node Gate. No public qnsd proxy. No auto-heal. Not anonymity. Not a Softwares-tab product. Author: Aziel Eliab only. Catalog MCP <code>mesh_*</code> + FragGate <code>slug=mesh</code>.</p>
...
async function handleRuntime(request, url) {
  ...
  if (path.startsWith("/v1/") || path === "/v1") {
    return json({ error: "not found", hint: "GET /v1/health /v1/models /v1/skill /v1/jeeves /v1/mesh ; POST /v1/lamb-check — hosted /v1 is lamb-check ONLY, never a paid-key proxy. Ask Jeeves chat is local azai serve. Suite mesh /v1/mesh/* PROXY (default OFF). QNS-CD-1.0 hub cite only — not a public qnsd proxy.", limitation: engine.LIMITATION, provider_proxy: false }, 404);
  }
  return null;
}
